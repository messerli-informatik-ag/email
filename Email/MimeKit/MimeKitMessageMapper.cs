...
namespace Messerli.Email.MimeKit
{
    internal sealed class MimeKitMessageMapper : IMimeKitMessageMapper
    {
        ...
        {
            MapToAutoSubmittedHeaderValue(autoGenerationType)
                .AndThen(headerValue => mimeMessage.Headers[AutoSubmittedHeaderName] = headerValue);
        }

        private static Option<string> MapToAutoSubmittedHeaderValue(AutoGenerationType autoGenerationType)
            => autoGenerationType switch
            {
                AutoGenerationType.Unspecified => Option<string>.None(),
                AutoGenerationType.AutoGenerated => Option.Some("auto-generated"),
                AutoGenerationType.AutoReplied => Option.Some("auto-replied"),
        ...
        private static TItem? ToNullable<TItem>(Option<TItem> option)
            where TItem : class
            => option.Match(some: v => v, none: (TItem?)null);
    }
}
